...
function execute(scenario, scenarioState, cb, doLog) {
    ...
    for (var deviceLoop = 0; deviceLoop < scenario.actuators.length; deviceLoop++) {
        ...
        conflictManager.checkAgainstResolvedConflicts(device, scenario, scenarioState, newcommand, function(passed, newcommand, device){
            if(passed) {
...
function stop(scenario, cb) {
    ...
    setTimeout(function(){
        var scenarioList = scenarios.filter(function(d){
    ...
    //    for (var i = 0; i < scenarios.length; i++) {
    //        var scenarioding = JSON.parse(JSON.stringify(scenarios[i]));
    //        if(scenarioding.status) {
    //            execute(scenarioding, 'start', function(yolo){
    //
    //            });
    //            //for(var y = 0; y < scenarioding.actuators.length; y++) {
    //            //
    //            //    var device = deviceManager.getActuator(scenarioding.actuators[y].deviceid);
    //            //    conflictManager.checkAgainstResolvedConflicts(device, scenario, true, scenarioding.actuators[y].action.command, function(passed, newcommand, device){
    //            //        console.log(passed);
    //            //        if(passed) {
    //            //            deviceManager.executeCommand(newcommand, device, {}, true);
    //            //        }
